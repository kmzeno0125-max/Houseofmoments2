...
import jacuzziFeatureImg from '../assets/jacuzzi.png';
import grillFeatureImg from '../assets/image copy copy.png';
...
import uploadedTerraceImg1 from '../assets/files_10287071-2026-05-27T12-55-07-257Z-image.webp';
import uploadedTerraceImg2 from '../assets/hero-bg.png';
import uploadedTerraceImg3 from '../assets/jacuzzikanape.png';
import uploadedTerraceImg4 from '../assets/kertjacuzzinaofeny.png';

const kitchenDiningImages = [kitchenDiningImg1, kitchenDiningImg2, kitchenDiningImg3];
const livingRoomImages = [
  '/assets/felszereltseg/felszereltseg_nappali1.jpeg',
  '/assets/felszereltseg/felszereltseg_nappali2.jpeg',
];
const livingRoomImageAlts = [
  'Nappali nagy méretű kanapéval és Smart TV-vel',
  'Otthonos nappali kanapéval és Smart TV-vel',
];
const transferredRoomImg = '/assets/felszereltseg/WhatsApp_Image_2026-05-30_at_07.17.38.jpeg';
const uploadedTerraceImages = [
  uploadedTerraceImg1,
  uploadedTerraceImg2,
  uploadedTerraceImg3,
  uploadedTerraceImg4,
];
const roomImages = [passionRoomImg, transferredRoomImg];
const roomImageAlts = [
  'Kényelmes, prémium szoba',
  'Passion szoba különleges hangulatvilágítással',
];
const uploadedGardenImages = [
  '/assets/felszereltseg/2026-08-21_16.11.16%20copy.jpg',
  '/assets/felszereltseg/2026-08-21_16.11.20%20copy.jpg',
  '/assets/felszereltseg/2026-08-21_16.11.22%20copy.jpg',
];
...
const bathroomImages = [
  '/gallery/houseofmomentsfurdoszoba1.jpeg',
  '/gallery/houseofmomentsfurdoszoba2.jpeg',
  '/gallery/houseofmomentswc1.jpeg',
];
const bathroomImageAlts = [
  'Fürdőszoba',
...
const blocksData: Record<'hu' | 'en', Block[]> = {
  hu: [
    ...
    },
    {
      id: 'nappali',
      title: 'Nappali',
      intro: 'A tágas, otthonos nappali nagyméretű kanapéval és Smart TV-vel biztosít kényelmes közösségi teret pihenéshez, filmnézéshez és közös beszélgetésekhez.',
      details: [
        'Tágas, kényelmes közösségi tér',
        'Nagyméretű sarokkanapé',
      ...
      image: '',
      imageAlt: 'Otthonos nappali',
      images: livingRoomImages,
      imageAlts: livingRoomImageAlts,
    },
    {
      id: 'konyha-etkezo',
      title: 'Konyha & Étkező',
    ...
    },
    {
      ...
      images: roomImages,
      imageAlts: roomImageAlts,
    },
    {
      id: 'furdoszoba-wc',
      title: 'Fürdőszoba és külön WC',
      intro: 'A fürdőszoba és a WC két különálló, egymás mellett található helyiség, így kényelmesebben használhatók.',
      details: [
        'Fürdőszoba zuhanyzóval',
        'Különálló WC',
        'Kéztörlő és fürdőlepedő',
        'Hajszárító',
      ],
      image: '',
      imageAlt: 'Fürdőszoba és külön WC',
      images: bathroomImages,
      imageAlts: bathroomImageAlts,
    },
    {
    ...
    {
      id: 'kozos-terek',
      title: 'A ház további felszereltsége',
      intro: 'Az alábbi kényelmi eszközök a teljes ház felszereltségéhez tartoznak.',
      details: [
        ...
        'Bakelit lemezjátszó',
        'Beltéri játékok felnőtteknek és gyermekeknek',
        'Ingyenes Wi-Fi a ház teljes területén és a kertben',
        'Vasaló',
        'Vasalódeszka',
        'Mosógép',
        'Ruhaszárító / teregető állvány',
      ],
      image: '',
  ...
  en: [
    ...
    },
    {
      id: 'nappali',
      title: 'Living Room',
      intro: 'The spacious and welcoming living room features a large sofa and Smart TV, providing a comfortable shared space for relaxing, watching films and spending time together.',
      details: [
        'Spacious and comfortable shared living area',
        'Large corner sofa',
        'Smart TV with Netflix and YouTube access',
        'Air conditioning',
        'Vinyl record player',
        'Indoor games for adults and children',
      ],
      image: '',
      imageAlt: 'Welcoming living room',
      images: livingRoomImages,
      imageAlts: ['Living room with a large sofa and Smart TV', 'Welcoming living room with sofa and Smart TV'],
    },
    {
      id: 'konyha-etkezo',
    ...
    },
    {
      ...
      images: roomImages,
      imageAlts: ['Comfortable, premium room', 'Passion room with atmospheric lighting'],
    },
    {
      id: 'furdoszoba-wc',
      title: 'Bathroom and Separate Toilet',
      intro: 'The bathroom and toilet are located in two separate, adjacent rooms for greater convenience.',
      details: [
      ...
      image: '',
      imageAlt: 'Bathroom and separate toilet',
      images: bathroomImages,
      imageAlts: ['Bathroom', 'Bathroom with shower', 'Separate toilet'],
    },
    {
      id: 'bogracs-grill-kert',
    ...
    },
    {
      ...
      intro: 'The following amenities are part of the entire house\'s equipment.',
      details: [
        ...
        'Vinyl record player',
        'Indoor games for adults and children',
        'Free Wi-Fi throughout the house and garden',
        'Iron',
        'Ironing board',
        'Washing machine',
...
function KitchenImageSlider({
  images,
  alts,
  previousLabel = 'Previous kitchen image',
  nextLabel = 'Next kitchen image',
}: {
  images: string[];
  alts: string[];
...
function FeatureBlock({ block, index }: { block: Block; index: number }) {
  ...
  const isImageSlider = block.id === 'nappali' || block.id === 'konyha-etkezo' || block.id === 'szobak' || block.id === 'furdoszoba-wc';
  ...
  if (isImageSlider && block.images && block.imageAlts) {
    return (
      <motion.article
        initial={{ opacity: 0, y: 40 }}
        whileInView={{ opacity: 1, y: 0 }}
      ...
      >
        <div className={`flex flex-col ${imageLeft ? 'lg:flex-row' : 'lg:flex-row-reverse'}`}>
          <div className="lg:w-[45%] flex-shrink-0">
            <KitchenImageSlider
              images={block.images}
              alts={block.imageAlts}
              previousLabel={block.id === 'nappali' ? (lang === 'en' ? 'Previous living room image' : 'Előző nappali kép') : undefined}
              nextLabel={block.id === 'nappali' ? (lang === 'en' ? 'Next living room image' : 'Következő nappali kép') : undefined}
            />
          </div>
          <div className="flex-1 p-6 sm:p-8 lg:p-10 flex flex-col justify-center">
            ...
            <AnimatePresence initial={false}>
              {open && (
                ...
                >
                  <ul className={`mt-5 pt-5 border-t border-gold/10 ${(block.id === 'konyha-etkezo' || block.id === 'bogracs-grill-kert') ? 'grid grid-cols-1 sm:grid-cols-2 gap-3' : 'space-y-3'}`}>
                    {block.details.map((item) => (
                      <li key={item} className="flex items-start gap-3">
                        <Check size={16} className="text-gold flex-shrink-0 mt-0.5" />
                        <span className="text-ink/80 text-sm">{item}</span>
          ...
          <div className="lg:w-[45%] flex-shrink-0">
            <div className="aspect-[4/3] lg:aspect-auto lg:h-full relative overflow-hidden">
              <img
                ...
                className="w-full h-full object-cover"
              />
              {block.id === 'terasz-jakuzzi' ? (
                <div className="absolute inset-0 bg-plum">
                  <img
                    src={uploadedTerraceImages[uploadedImageIndex]}
                    alt="Terasz és jacuzzi"
                    className="w-full h-full object-contain"
                  />
                  <button
                    type="button"
                    onClick={() => setUploadedImageIndex((current) => (current + uploadedTerraceImages.length - 1) % uploadedTerraceImages.length)}
                    aria-label="Előző terasz és jacuzzi kép"
                    className="absolute left-3 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full bg-black/40 backdrop-blur-sm border border-white/30 flex items-center justify-center text-white hover:bg-black/60 hover:scale-110 transition-all duration-200"
                  >
                    <ChevronLeft size={18} />
                  </button>
                  <button
                    type="button"
                    onClick={() => setUploadedImageIndex((current) => (current + 1) % uploadedTerraceImages.length)}
                    aria-label="Következő terasz és jacuzzi kép"
                    className="absolute right-3 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full bg-black/40 backdrop-blur-sm border border-white/30 flex items-center justify-center text-white hover:bg-black/60 hover:scale-110 transition-all duration-200"
                  >
                    <ChevronRight size={18} />
                  </button>
                  <span className="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 rounded-full bg-black/45 px-3 py-1 text-xs font-medium text-white backdrop-blur-sm">
                    {uploadedImageIndex + 1} / {uploadedTerraceImages.length}
                  </span>
                </div>
              ) : block.id === 'bogracs-grill-kert' ? (
                <div className="absolute inset-0 bg-plum">
                  <img
                    src={uploadedGardenImages[gardenImageIndex]}
                    alt="Kert és grill"
                    className="w-full h-full object-contain"
                  />
                  <button
                    type="button"
                    onClick={() => setGardenImageIndex((current) => (current + uploadedGardenImages.length - 1) % uploadedGardenImages.length)}
                    aria-label="Előző kerti kép"
                    className="absolute left-3 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full bg-black/40 backdrop-blur-sm border border-white/30 flex items-center justify-center text-white hover:bg-black/60 hover:scale-110 transition-all duration-200"
                  >
                    <ChevronLeft size={18} />
                  </button>
                  <button
                    type="button"
                    onClick={() => setGardenImageIndex((current) => (current + 1) % uploadedGardenImages.length)}
                    aria-label="Következő kerti kép"
                    className="absolute right-3 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full bg-black/40 backdrop-blur-sm border border-white/30 flex items-center justify-center text-white hover:bg-black/60 hover:scale-110 transition-all duration-200"
                  >
                    <ChevronRight size={18} />
                  </button>
                  <span className="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 rounded-full bg-black/45 px-3 py-1 text-xs font-medium text-white backdrop-blur-sm">
                    {gardenImageIndex + 1} / {uploadedGardenImages.length}
                  </span>
                </div>
              ) : (
                <div className="absolute inset-0 bg-gradient-to-t from-plum/40 to-transparent" />
...
export default function FelszereltsegPage() {
  ...
  const t = {
    ...
    heroSub: lang === 'en'
      ? 'Discover the comfort of our guest house: from the private jacuzzi and welcoming living room to the well-equipped kitchen and atmospheric selfie corner.'
      : 'Fedezd fel vendégházunk kényelmét: a privát jacuzzitól az otthonos nappalin és a jól felszerelt konyhán át egészen a hangulatos szelfisarokig.',
